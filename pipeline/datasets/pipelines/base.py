...
class BaseRangeLimit(object):
    def __init__(self, x_range=None, y_range=None, z_range=None):
        ...
        # 提供有效值的轴
        valid_axis_list = [
            i for (i, axis_range) in enumerate(range_list) if axis_range is not None
        ]
        self.valid_axis = valid_axis_list
        self.range_list = [
            sorted(range_list[valid_axis]) for valid_axis in valid_axis_list
        ]
    ...
    def get_limit_range_indices(self, pointcloud):
        ...
        for valid_axis, axis_range in zip(self.valid_axis, self.range_list):
            mask_list.append(
                np.logical_and(
                    pointcloud[:, valid_axis] >= axis_range[0],
        ...
        if N >= self.num_points:
            ...
            if self.allow_less_points:
                ...
                sample_idx = np.concatenate(
                    [
                        np.repeat(np.arange(N), repeat_times),
                        np.random.choice(N, sample_num, replace=False),
                    ],
                    axis=-1,
                )
        return sample_idx
...
class BaseTransform(object):
    ...
    def transformer_tr(self, pointcloud, Tr):
        ...
        pointcloud = np.concatenate(
            [pointcloud[:, :3], np.ones((pointcloud.shape[0], 1))], axis=-1
        )
